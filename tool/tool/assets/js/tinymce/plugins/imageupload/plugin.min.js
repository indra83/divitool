        ...
        init : function(ed, url) {
            ...
            ed.addCommand('mceImageUpload', function() {
                ...
                var container = '\
                    <div class="imageUploadContainer mce-container mce-panel mce-window">\
                        <div class="imageUploadContainerInner">\
                            ...
                            <form action="/savefile/blah" method="POST"  enctype="multipart/form-data" id="uploadImageForm">\
                ...
                $('.imageUploadSubmit').on('click', function(){
                    ...
                    // $('#uploadImageForm').attr('action','/savefile/'+ed.chapterid+"/"+ed.topicid+"/"+($('#image-upload-area').val().split('\\')[$('#image-upload-area').val().split('\\').length-1]));
                    if ($('#image-upload-area').val() != '') {
                        $('body').append('<div class="imageUploadFg"></div>');
                        $('body').append('<div class="imageUploadFgLoading"></div>');
                        // $('#uploadImageForm').submit();

                        var input=$('#image-upload-area')[0];
                    if ( input.files && input.files[0] ) {
                        var FR= new FileReader();
                        FR.onload = function(e) {
                             // $('#img').attr( "src", e.target.result );
                             var tpl = '<img src="%s" />';
                                    ed.insertContent(tpl.replace('%s', e.target.result));
                                    ed.focus();
                                    removeForeground();
                                    removeBackground();
