...
var nocookies = false;
var Cookie = {set: function(c_name, value, expiredays) {
        if (nocookies)
            return;
        {
            var exdate = new Date();
...
var URL = {get: function(type, default_val) {
        if (location.href.match(type + '='))
            return location.href.split(type + '=')[1].split('&')[0];
        return default_val;
    }};
var Opacity = {set: function(id, opacity) {
...
var Panel = {plock: null,ctimer: null,otimer: null,oid: null,timer: Array(),open: function(a) {
        if (a) {
            var id = a.id;
            if (this.timer[id] != '') {
    ...
    },hoverdiv: null,hlock: false,hover: function(img, e) {
        if (this.hoverdiv) {
            this.lock = true;
        ...
        if (!this.hlock)
            CCgetId('hover').style.display = 'none';
...
function EqTextArea(preview, input, comment, download, intro) {
    ...
    this.setText = function(val) {
        ...
        do {
            ...
            if (i != -1) {
                ...
                if (subtex.substr(0, 4) == '\\bg_' && EqEditor.setSelIndx('bg', subtex.substr(4)))
                    go = 1;
                if (subtex.substr(0, 4) == '\\fn_' && EqEditor.setSelIndx('font', subtex.substr(4)))
                    go = 1;
    ...
    this.renderEqn = function(callback) {
        ...
        if (bracket == 0) {
            ...
            val = "%5Cdpi%7B150%7D%20%5Clarge%20"+val;
            ...
            if (this.changed) {
                ...
                switch (format) {
                    ...
                    case 'pdf':
                        ...
                        break;
                    case 'swf':
                        img.src = EDITOR_SRC + '/images/spacer.gif';
                        var str = AC_FL_RunContent('codebase', EDITOR_SW_FLASH, 'width', '600', 'height', '100', 'src', (EQUATION_ENGINE + '/swf.latex?' + val), 'quality', 'high', 'pluginspage', EDITOR_SW_PLAYER, 'align', 'top', 'scale', 'showall', 'wmode', 'window', 'devicefont', 'false', 'bgcolor', '#ffffff', 'menu', 'true', 'allowFullScreen', 'true', 'movie', (EQUATION_ENGINE + '/swf.latex?' + val));
                        this.setcomment(str);
                        this.setdownload('<a target="_blank" href="' + EQUATION_ENGINE + '/swf.download?' + sval + '">Click here to Download Equation (SWF)</a>');
                        break;
                    case 'emf':
                        img.src = EQUATION_ENGINE + '/gif.latex?' + val;
                        this.setdownload('<a target="_blank" href="' + EQUATION_ENGINE + '/emf.download?' + sval + '"><img src="images/emf.jpg" width="30" height="30" align="middle" /> Click here to Download Equation (Windows EMF)</a>');
                        break;
                }
...
;
var gallery = null;
var Example = {lastbutton: null,load_json: function(file, text) {
...
var ExportButton = {bArray_id: new Array(),bArray_area: new Array(),bArray_mode: new Array(),bArray_fn: new Array(),bsize: 0,state: function(state) {
        for (i = 0; i < this.bsize; i++) {
            if (state)
                this.bArray_id[i].className = 'lightbluebutton';
            else
...
var EqEditor = {SID: 0,copy_button: null,key_text: '',format: 'png',setSelIndx: function(id, v) {
        var s = CCgetId(id);
        if (s)
        ...
        return false;
    },targetArray: new Array(),targetSize: 0,targetArea: null,curTarget: 0,changeExportArea: function(id, mode) {
        ...
        this.targetSize++;
    },editor_id: null,embed: function(id, SID, design, language) {
        if (this.targetSize > 0) {
    ...
    },targetFn: null,copyToTarget: function(text) {
        ...
        CConchange('type', function(e) {
            var type = CCgetId('type');
            if (type) {
                Cookie.set('type', type.value, 10);
            }
            EqEditor.changeExportArea('eqcoderaw', type.value);
        ...
        return norm.replace("{$TEXT}", text);
    },extendchar: null,countclick: function() {
        this.targetArea.countclick();
        var a = CCgetId('redobutton');
        if (a)
            a.src = EDITOR_SRC + "/images/buttons/redo-x.gif";
...
var ZeroClipboard = {version: "1.0.4",clients: {},moviePath: 'js/ZeroClipboard.swf',nextId: 1,$: function(thingy) {
        if (typeof (thingy) == 'string')
            thingy = document.getElementById(thingy);
        if (!thingy.ccaddClass) {
            ...
            thingy.ccaddClass = function(name) {
                this.ccremoveClass(name);
                this.className += ' ' + name;
            };
            thingy.ccremoveClass = function(name) {
...
ZeroClipboard.Client.prototype = {id: 0,ready: false,movie: null,clipText: '',handCursorEnabled: true,cssEffects: true,handlers: null,div: null,domElement: null,glue: function(elem) {
        ...
        switch (eventName) {
            ...
            case 'mouseover':
                if (this.domElement && this.cssEffects) {
                    this.domElement.ccaddClass('hover');
                    if (this.recoverActive)
                        this.domElement.ccaddClass('active');
                }
                break;
            case 'mouseout':
                if (this.domElement && this.cssEffects) {
                    ...
                    if (this.domElement.hasClass('active')) {
                        this.domElement.ccremoveClass('active');
                        this.recoverActive = true;
                    }
                    this.domElement.ccremoveClass('hover');
                }
                break;
            case 'mousedown':
                if (this.domElement && this.cssEffects) {
...
function clipboard_init() {
    latexclip = new ZeroClipboard.Client();
    latexclip.setHandCursor(true);
    latexclip.addEventListener('mouseOver', latex_mouse_over);
    //latexclip.glue('latex_copy');
    htmlclip = new ZeroClipboard.Client();
    htmlclip.setHandCursor(true);
    htmlclip.addEventListener('mouseOver', html_mouse_over);
    //htmlclip.glue('html_copy');
}
function latex_mouse_over(client) {
    latexclip.setText($$('latex_formula').value);
}
function html_mouse_over(client) {
    htmlclip.setText($$('eqcoderaw').value);
}
...
Scroll.prototype = {init: function(maindiv, leftarrow, rightarrow, overview, newpanel_php) {
        this.panels = 0;
        this.maxpanels = 0;
        this.speed = 10;
        this.pause = 2;
